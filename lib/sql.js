...
SQLConnector.prototype.columnEscaped = function(model, property) {
  var dbMeta = this.getConnectorSpecificSettings(model);
  var columnPrefix = dbMeta.function ? dbMeta.table : this.tableEscaped(model);
  return columnPrefix + '.' + this.escapeName(this.column(model, property));
};
...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  ...
  var selectStmt = (dbMeta.function) ?
    new ParameterizedSQL({
      sql: 'SELECT ' +
      this.buildColumnNames(model, filter) + distinct +
      ' FROM ' + dbMeta.function + ' AS ' + dbMeta.table,
      params: [filter.functionParam]
    })
    :
    new ParameterizedSQL('SELECT ' + distinct +
  ...
  if (filter) {
    ...
    if (options.skipOrderBy !== true && filter.order) {
...
SQLConnector.prototype.buildJoins = function(model, where) {
  ...
  var buildOneToMany = function buildOneToMany(relName, modelFrom, keyFrom, modelTo, keyTo, filter) {
    var modelToEscaped = this.tableEscaped(modelTo);
    var relNameEscaped = this.escapeName(relName);
    var innerFilter = assign({}, filter);
    var innerIdField = {};
    innerIdField[keyTo] = true;
    innerFilter.fields = assign({}, innerFilter.fields, innerIdField);
    innerFilter.order = null;

    var condition = this.columnEscaped(modelFrom, keyFrom) + '=' +
    relNameEscaped + '.' + this.escapeName(this.column(modelTo, keyTo));
  ...
  for (var key in where) {
    ...
    if (!rel.modelThrough) {
      // 1:n relation
      join = buildOneToMany(key, model, keyFrom, modelTo, keyTo, where[key]);
    } else {
      // n:m relation
      ...
      var joinInner = buildOneToMany(key, model, keyFrom, modelThrough, keyTo, innerFilter);
      join = buildOneToMany(key + '_' + modelThroug, modelThrough, keyThrough, modelTo, modelToKey, where[key]);
...
SQLConnector.prototype.buildCount = function(model, where, options) {
  ...
  if (dbMeta.function) {
    // Extract the TVF function parameters and then delete them from the where clause
    functionParam = where.functionParam;
    delete where.functionParam;
  }
  ...
  // TVF support
  var stmt = (dbMeta.function) ?
    new ParameterizedSQL({ sql: 'SELECT ' + count + ' as "cnt" FROM ' + dbMeta.function + ' AS ' + dbMeta.table, params: [where.functionParam]})
    :
    new ParameterizedSQL('SELECT ' + count + ' as "cnt" FROM ' + this.tableEscaped(model))
